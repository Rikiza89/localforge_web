...
# .localforgeディレクトリ名 — shared constant from domain
...
class AnalysisService:
    ...
    def read_file_chunk(self, path: Path, root: Path) -> FileChunk:
        ...
        - ≤350行: フル読み込み (strategy=full)
        - >350行: 先頭150行 + 末尾150行 + 構造的ランドマーク (strategy=hybrid)
    ...
    def build_tree_text(self, root: Path, prefix: str = "", path: Optional[Path] = None) -> str:
        """
        ディレクトリツリーをUnicodeボックス文字で表現するメソッド。
        ...
        for i, entry in enumerate(filtered):
            ...
            if entry.is_dir():
                ...
                subtree = self.build_tree_text(root, prefix + extension, entry)
                if subtree:
    ...
    def _get_top_chunks_by_keywords(
        self,
        chunks: List[FileChunk],
        query: str,
        top_n: int = 5,
